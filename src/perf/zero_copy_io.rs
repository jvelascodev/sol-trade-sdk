...
impl SharedMemoryPool {
    ...
    pub fn new(pool_id: u32, total_size: usize, block_size: usize) -> Result<Self> {
        ...
        log::info!(
            "🚀 Created shared memory pool {} with {} blocks of {} bytes each",
            pool_id,
            total_blocks,
            aligned_block_size
        );
...
impl ZeroCopyBlock {
    ...
    /// The caller must ensure that the block is still valid and not being
    ...
    ///
    /// This function is unsafe because it performs raw pointer manipulation.
    /// The caller must ensure that `data.len()` does not exceed the block size
    /// and that the destination memory is valid.
    #[inline(always)]
    ...
    ///
    /// This function is unsafe because it performs raw pointer manipulation.
    /// The caller must ensure that `len` does not exceed the block size
    /// and that the memory is valid.
    #[inline(always)]
...
impl ZeroCopyStats {
    ...
    pub fn print_stats(&self) {
        ...
        log::info!(
            "   📦 Blocks: Allocated={}, Freed={}, Active={}",
            allocated,
            freed,
            allocated.saturating_sub(freed)
        );
        log::info!("   📊 Bytes Transferred: {} ({:.2} MB)", bytes, bytes as f64 / 1024.0 / 1024.0);
        log::info!(
            "   💾 Memory Mapped Usage: {} ({:.2} MB)",
            mmap_usage,
            mmap_usage as f64 / 1024.0 / 1024.0
        );
    }
}
