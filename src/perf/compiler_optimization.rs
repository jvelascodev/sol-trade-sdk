...
impl SIMDCompileTimeOptimizer {
    ...
    /// # Safety
    ///
    /// This function is unsafe because it uses SIMD intrinsics and performs unchecked
    /// pointer arithmetic. The caller must ensure that the `avx2` target feature is
    /// enabled and that the pointer is valid.
    #[cfg(target_arch = "x86_64")]
